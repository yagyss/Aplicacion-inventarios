...
const INIT_PRODUCTS = [
  ...
  { id: 3, name: "Pantalón Cargo", sku: "PAN-001", brand: "Adidas", color: "Khaki", size: "32", stock: 3, minStock: 5, price: 120000, cost: 65000, sold: 45, category: "Ropa", emoji: "👖" },
  { id: 4, name: "Sneaker Urban", sku: "ZAP-001", brand: "Vans", color: "Blanco", size: "42", stock: 15, minStock: 8, price: 280000, cost: 140000, sold: 67, category: "Calzado", emoji: "👟" },
  { id: 5, name: "Sneaker Urban", sku: "ZAP-002", brand: "Vans", color: "Rojo", size: "41", stock: 2, minStock: 5, price: 280000, cost: 140000, sold: 89, category: "Calzado", emoji: "👟" },
  { id: 6, name: "Hoodie Premium", sku: "HOO-001", brand: "Champion", color: "Gris", size: "XL", stock: 22, minStock: 8, price: 180000, cost: 90000, sold: 55, category: "Ropa", emoji: "🧥" },
  { id: 7, name: "Gorra Snapback", sku: "GOR-001", brand: "New Era", color: "Azul", size: "Único", stock: 30, minStock: 5, price: 85000, cost: 40000, sold: 200, category: "Accesorios", emoji: "🧢" },
];

const INIT_SALES = [
  { id: 1, productId: 7, qty: 5, total: 425000, date: "2025-02-01", method: "Efectivo" },
  { id: 2, productId: 4, qty: 1, total: 280000, date: "2025-02-05", method: "Tarjeta" },
  { id: 3, productId: 1, qty: 2, total: 90000, date: "2025-02-10", method: "Nequi" },
  { id: 4, productId: 5, qty: 1, total: 280000, date: "2025-02-14", method: "Tarjeta" },
  { id: 5, productId: 6, qty: 3, total: 540000, date: "2025-02-18", method: "Transferencia" },
  { id: 6, productId: 3, qty: 2, total: 240000, date: "2025-02-22", method: "Efectivo" },
  { id: 7, productId: 7, qty: 8, total: 680000, date: "2025-02-25", method: "Tarjeta" },
  { id: 8, productId: 2, qty: 4, total: 180000, date: "2025-02-27", method: "Nequi" },
];
...
const INIT_EXPENSES = [
  ...
  { id: 4, concept: "Servicios públicos", amount: 85000, date: "2025-02-05", category: "Operacional", emoji: "💡" },
  { id: 5, concept: "Empaque y bolsas", amount: 45000, date: "2025-02-10", category: "Operacional", emoji: "🛒" },
  { id: 6, concept: "Pauta Facebook Ads", amount: 200000, date: "2025-02-20", category: "Marketing", emoji: "📱" },
];
...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .toast { position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); background: #1A1A2E; color: white; border-radius: 16px; padding: 13px 22px; font-weight: 700; font-size: 14px; z-index: 200; white-space: nowrap; box-shadow: 0 8px 24px rgba(0,0,0,0.2); animation: popIn 0.3s ease; }
  ...
  return (
    <div style={{ fontFamily: "'Nunito','Segoe UI',sans-serif", background: C.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative", paddingBottom: 90 }}>
      ...
      <div style={{ background: C.white, padding: "18px 20px 14px", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 1px 0 " + C.border }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div style={{ width: 38, height: 38, background: "linear-gradient(135deg,#00C896,#4A90FF)", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, color: "white", fontSize: 16 }}>A</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "18px 16px" }}>
        {tab === "home" && <Home products={products} sales={sales} totalSales={totalSales} totalExpenses={totalExpenses} profit={profit} lowStock={lowStock} setTab={setTab} setModal={setModal} />}
        ...
        {tab === "sales" && <Sales sales={sales} products={products} totalSales={totalSales} setModal={setModal} />}
        {tab === "expenses" && <Expenses expenses={expenses} setExpenses={setExpenses} totalExpenses={totalExpenses} showToast={showToast} setModal={setModal} />}
        {tab === "finance" && <Finance products={products} sales={sales} expenses={expenses} totalSales={totalSales} totalExpenses={totalExpenses} profit={profit} />}
        {tab === "metrics" && <Metrics products={products} sales={sales} totalSales={totalSales} profit={profit} />}
      </div>
...
// ── HOME ──────────────────────────────────────────────────────────────────────
function Home({ products, totalSales, totalExpenses, profit, lowStock, setTab, setModal }) {
...
// ── FINANCE MODULE ────────────────────────────────────────────────────────────
function Finance({ products, sales, expenses, totalSales, totalExpenses, profit }) {
  const [activeSection, setActiveSection] = useState("cashflow");

  // ── Cálculos clave ──
  const marketingExpenses = expenses.filter(e => e.category === "Marketing").reduce((a, e) => a + e.amount, 0);
  const operationalExpenses = expenses.filter(e => e.category === "Operacional").reduce((a, e) => a + e.amount, 0);
  const purchaseExpenses = expenses.filter(e => e.category === "Compras").reduce((a, e) => a + e.amount, 0);
  const totalTransactions = sales.length;

  // CPA = Gasto en marketing / número de ventas
  const cpa = totalTransactions > 0 ? marketingExpenses / totalTransactions : 0;

  // Costo variable total = costo de productos vendidos
  ...
  // Flujo de caja por semana simulado
  const cashflowDays = [
    { label: "Sem 1", entrada: sales.filter(s => s.date <= "2025-02-07").reduce((a, s) => a + s.total, 0), salida: expenses.filter(e => e.date <= "2025-02-07").reduce((a, e) => a + e.amount, 0) },
    { label: "Sem 2", entrada: sales.filter(s => s.date > "2025-02-07" && s.date <= "2025-02-14").reduce((a, s) => a + s.total, 0), salida: expenses.filter(e => e.date > "2025-02-07" && e.date <= "2025-02-14").reduce((a, e) => a + e.amount, 0) },
    { label: "Sem 3", entrada: sales.filter(s => s.date > "2025-02-14" && s.date <= "2025-02-21").reduce((a, s) => a + s.total, 0), salida: expenses.filter(e => e.date > "2025-02-14" && e.date <= "2025-02-21").reduce((a, e) => a + e.amount, 0) },
    { label: "Sem 4", entrada: sales.filter(s => s.date > "2025-02-21").reduce((a, s) => a + s.total, 0), salida: expenses.filter(e => e.date > "2025-02-21").reduce((a, e) => a + e.amount, 0) },
  ];
  ...
  const frozenCapital = products.reduce((a, p) => a + p.stock * p.cost, 0);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="card" style={{ padding: 20 }}>
            ...
            <div style={{ background: C.bg, borderRadius: 12, padding: 14 }}>
              ...
              <div className="cf-row">
                <span style={{ fontSize: 13 }}>Gastas en marketing</span>
                <span style={{ fontWeight: 900, color: C.purple }}>{fmt(marketingExpenses)}</span>
              </div>
              <div className="cf-row">
                <span style={{ fontSize: 13 }}>Número de ventas</span>
                <span style={{ fontWeight: 900 }}>{totalTransactions}</span>
              </div>
              <div className="cf-row" style={{ borderBottom: "none" }}>
                <span style={{ fontSize: 13, fontWeight: 800 }}>Te cuesta conseguir cada venta</span>
                <span style={{ fontWeight: 900, color: C.purple }}>{fmt(cpa)}</span>
              </div>
            </div>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              ...
              <div style={{ background: C.orangeLight, borderRadius: 14, padding: "10px 14px" }}>
                <div style={{ fontSize: 24 }}>⚖️</div>
              </div>
            </div>
            <div style={{ background: C.bg, borderRadius: 12, padding: 14 }}>
              <div className="cf-row">
                <span style={{ fontSize: 13 }}>Costos fijos mensuales</span>
                <span style={{ fontWeight: 900, color: C.orange }}>{fmt(fixedCosts)}</span>
              </div>
              <div className="cf-row">
                <span style={{ fontSize: 13 }}>Margen bruto actual</span>
                <span style={{ fontWeight: 900 }}>{grossMargin}%</span>
              </div>
              <div className="cf-row" style={{ borderBottom: "none" }}>
                <span style={{ fontSize: 13, fontWeight: 800 }}>Ya vendiste este mes</span>
                <span style={{ fontWeight: 900, color: totalSales >= breakEven ? C.green : C.red }}>{fmt(totalSales)}</span>
              </div>
            </div>
              ...
              <div key={m.label} style={{ marginBottom: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted, fontWeight: 600 }}>{m.desc}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 900, fontSize: 20, color: m.color }}>{m.value}%</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: m.color }}>{fmt(m.amount)}</div>
                  </div>
                </div>
                <div className="bar" style={{ height: 10 }}>
                  <div className="bar-fill" style={{ width: `${Math.max(0, Math.min(100, m.value))}%`, background: m.color }} />
                </div>
              </div>
            ))}
            ...
            <div style={{ background: C.bg, borderRadius: 14, padding: 14, marginTop: 6 }}>
              ...
              {[
                ...
                <div key={r.label} className="cf-row">
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 13, fontWeight: 900 }}>{r.value}</div>
                    <div style={{ fontSize: 11, color: C.muted }}>{r.status}</div>
                  </div>
                </div>
              ))}
            </div>
...
// ── INVENTORY ─────────────────────────────────────────────────────────────────
function Inventory({ products, setProducts, lowStock, showToast, setModal }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div><div style={{ fontWeight: 900, fontSize: 20 }}>Inventario</div><div style={{ fontSize: 13, color: C.muted, fontWeight: 600 }}>{products.length} refs · {products.reduce((a, p) => a + p.stock, 0)} uds</div></div>
      ...
      <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
        {brands.map(b => <button key={b} className={`filter-btn ${filter === b ? "active" : ""}`} onClick={() => setFilter(b)}>{b}</button>)}
      </div>
      {filtered.map(p => {
          ...
          <div key={p.id} className="card" style={{ padding: 16, border: `2px solid ${isLow ? C.red + "40" : "transparent"}` }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 14, paddingTop: 12, borderTop: "1.5px solid " + C.border }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button onClick={() => { setProducts(prev => prev.filter(x => x.id !== p.id)); showToast("🗑️ Eliminado"); }} style={{ background: C.redLight, border: "none", borderRadius: 10, padding: "8px 12px", cursor: "pointer", fontWeight: 800, fontSize: 13, color: C.red, fontFamily: "inherit" }}>Borrar</button>
              </div>
            </div>
          </div>
        );
...
// ── SALES ─────────────────────────────────────────────────────────────────────
function Sales({ sales, products, totalSales, setModal }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {Object.entries(byMethod).map(([m, total]) => {
          const [color, bg] = colors[m] || [C.muted, C.bg];
          return <div key={m} className="card" style={{ padding: 16 }}><div style={{ fontSize: 13, fontWeight: 800, color, marginBottom: 4 }}>{m}</div><div style={{ fontSize: 20, fontWeight: 900 }}>{fmt(total)}</div></div>;
        })}
      </div>
      <div className="card" style={{ padding: 18 }}>
...
// ── EXPENSES ──────────────────────────────────────────────────────────────────
function Expenses({ expenses, setExpenses, totalExpenses, showToast, setModal }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {Object.entries(byCategory).map(([cat, amount]) => (
          <div key={cat} className="card" style={{ padding: 16 }}><div style={{ fontSize: 13, fontWeight: 800, color: C.orange, marginBottom: 4 }}>{cat}</div><div style={{ fontSize: 20, fontWeight: 900 }}>{fmt(amount)}</div></div>
        ))}
      </div>
      <div className="card" style={{ padding: 18 }}>
        <div className="section-title">Historial</div>
...
// ── METRICS ───────────────────────────────────────────────────────────────────
function Metrics({ products, sales, totalSales, profit }) {
  ...
  const maxBrand = Math.max(...Object.values(byBrand));
  const maxColor = Math.max(...Object.values(byColor));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div className="card" style={{ padding: 18 }}>
        ...
        {Object.entries(byBrand).sort((a, b) => b[1] - a[1]).map(([brand, sold]) => (
          <div key={brand} style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}><span style={{ fontWeight: 800, fontSize: 14 }}>{brand}</span><span style={{ fontWeight: 900, color: C.blue }}>{sold} uds</span></div>
            <div className="bar"><div className="bar-fill" style={{ width: `${Math.round((sold / maxBrand) * 100)}%`, background: "linear-gradient(90deg,#4A90FF,#8B5CF6)" }} /></div>
          </div>
        ))}
      </div>
      <div className="card" style={{ padding: 18 }}>
        ...
        {Object.entries(byColor).sort((a, b) => b[1] - a[1]).map(([color, sold]) => (
          <div key={color} style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}><span style={{ fontWeight: 800, fontSize: 14 }}>{color}</span><span style={{ fontWeight: 900, color: C.purple }}>{sold} uds</span></div>
            <div className="bar"><div className="bar-fill" style={{ width: `${Math.round((sold / maxColor) * 100)}%`, background: "linear-gradient(90deg,#F472B6,#8B5CF6)" }} /></div>
          </div>
        ))}
      </div>
      {toBuy.length > 0 && (
...
function AddProductModal({ onClose, onSave }) {
  ...
  const emojis = { Ropa: "👕", Calzado: "👟", Accesorios: "🧢", Otro: "📦" };
  function save() {
    if (!f.name || !f.sku || !f.stock) return;
    onSave({ ...f, id: Date.now(), stock: +f.stock, minStock: +f.minStock, price: +f.price || 0, cost: +f.cost || 0, sold: 0, emoji: emojis[f.category] || "📦" });
  }
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="sheet">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[{ label: "Nombre *", key: "name", placeholder: "Ej: Camiseta Básica" }, { label: "SKU *", key: "sku", placeholder: "Ej: CAM-001" }, { label: "Marca", key: "brand", placeholder: "Ej: Nike" }, { label: "Color", key: "color", placeholder: "Ej: Blanco" }, { label: "Talla", key: "size", placeholder: "Ej: M, 42, Único" }, { label: "Stock inicial *", key: "stock", placeholder: "0", type: "number" }, { label: "Stock mínimo", key: "minStock", placeholder: "5", type: "number" }, { label: "Precio venta ($)", key: "price", placeholder: "0", type: "number" }, { label: "Costo ($)", key: "cost", placeholder: "0", type: "number" }].map(field => (
            <div key={field.key}>
              <div style={{ fontSize: 12, fontWeight: 800, color: C.muted, marginBottom: 5, textTransform: "uppercase" }}>{field.label}</div>
              <input className="stk-input" type={field.type || "text"} placeholder={field.placeholder} value={f[field.key]} onChange={e => setF(p => ({ ...p, [field.key]: e.target.value }))} />
            </div>
          ))}
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: C.muted, marginBottom: 5, textTransform: "uppercase" }}>Categoría</div>
...
function AddSaleModal({ products, onClose, onSave }) {
  ...
  function save() {
    ...
    onSave({ id: Date.now(), productId: +pid, qty: +qty, total, date: new Date().toISOString().split("T")[0], method }, +pid, +qty);
  }
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="sheet">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <select className="stk-input" value={pid} onChange={e => setPid(e.target.value)}>
              ...
              {products.filter(p => p.stock > 0).map(p => <option key={p.id} value={p.id}>{p.emoji} {p.name} — {p.color}/{p.size} ({p.stock} disp.) — {fmt(p.price)}</option>)}
            </select>
          </div>
          <div>
...
function AddExpenseModal({ onClose, onSave }) {
  ...
  const catEmojis = { Operacional: "🏪", Compras: "🛍️", Marketing: "📱", Logística: "🚚", Otro: "💡" };
  function save() {
    if (!f.concept || !f.amount) return;
    onSave({ ...f, id: Date.now(), amount: +f.amount, date: new Date().toISOString().split("T")[0], emoji: catEmojis[f.category] || "💡" });
  }
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="sheet">
